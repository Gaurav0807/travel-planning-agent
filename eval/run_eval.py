...
from knowledge_base import get_destination_info

sys.path.insert(0, "../eval")
from eval_criteria import build_correctness_metric
from eval_tracker import EvalTracker
from rouge_metric import compute_rouge1


def get_agent_response(user_input):
    """Run trip_analyzer and get response with token usage"""
...
def load_goldens():
    ...
    test_cases = []
    with open("../eval/goldens.csv", "r") as f:
...
def run_eval():
    ...
    print("\n" + "=" * 60)
    print("  RUNNING TRIP ANALYZER EVALUATION")
    print("=" * 60)
    ...
    tracker = EvalTracker()
    tracker.start_run(description="trip_analyzer_eval")
    ...
    for i, golden in enumerate(goldens):
        ...
        # Get agent response + tokens
        start = time.time()
        actual_output, tokens = get_agent_response(golden["input"])
    ...
    avg_rouge = sum(r["f1"] for r in rouge_scores) / len(rouge_scores) if rouge_scores else 0
